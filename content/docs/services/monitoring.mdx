...
- **Alarms auto-evaluate on write.** Every `PutMetricData` runs `evaluateAlarms()` for each affected namespace+metric, transitioning alarms between `OK` and `ALARM` against their threshold — so you assert alarm state by controlling the datapoints you push, not by waiting on a timer.
- **Auto-metrics from compute.** `RunInstances` automatically emits five metrics per instance (`CPUUtilization`, `NetworkIn`, `NetworkOut`, `DiskReadOps`, `DiskWriteOps`) with five backfilled datapoints at one-minute intervals, so newly launched instances have realistic history to query and alarm on.
- **Alarm history.** `GetAlarmHistory` returns each state transition with a timestamp — drive the timestamps deterministically with the [fake clock](/docs/features/fake-clock) rather than wall time.
