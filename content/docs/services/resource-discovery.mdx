...
Run cloudemu as a server and point the real Resource Explorer client at it — this is the path that exercises the actual discovery API and its query-string filters. Start it with `cloudemu serve` (or `docker run --rm -p 4566:4566 ghcr.io/stackshy/cloudemu`), which serves the AWS API on `http://localhost:4566`:

```bash
cloudemu serve   # AWS API on http://localhost:4566
```

Then point the Resource Explorer client at that endpoint and search across the resources you have created:
...
The Azure and GCP equivalents are `armresourcegraph` and `cloudasset/v1`, pointed at their endpoints the same way.

### In-process (Go unit tests)

For Go unit tests written inside cloudemu-aware code, stand the same wire server up in-process with `httptest.NewServer` and point the client at `ts.URL`. The `Drivers` you pass name exactly the drivers this engine reads from, alongside the account identity it reports:
...
defer ts.Close()
// point the same resourceexplorer2 client at ts.URL instead of the running endpoint
```
