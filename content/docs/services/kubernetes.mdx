...
Create **EKS**, **AKS**, and **GKE** clusters with the real SDKs, then drive the in-memory data plane with `client-go` or `kubectl`. Two layers, both shipped: the three control planes plus a shared **in-memory Kubernetes data-plane API server**.
