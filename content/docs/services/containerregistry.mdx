...
Reach for it when your code creates repositories, pushes or lists images, or depends on lifecycle cleanup or scan results — so you can exercise those paths without a real registry or real image layers. You store a manifest (digest, tags, size), not actual bytes, which is all the emulator needs.
...
| Manifests, not bytes | Images are stored as manifests (digest, tags, size, layers); there's no layer blob storage, which is all a control-plane emulator needs. |
