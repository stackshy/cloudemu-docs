...
Reach for it in tests when your code registers a model, stands up an endpoint, kicks off a training/tuning job, or writes to a feature store — so you can exercise that wiring without a live cluster. Asynchronous jobs complete synchronously to a terminal state, so Describe/List are deterministic and there are no polling loops in tests. For the generative side — Bedrock and Azure OpenAI — see [Generative AI](/docs/services/ai).
