...
Then point the stock RDS client at that endpoint and provision a standalone Postgres instance, an Aurora cluster, then stop the instance — the same lifecycle calls your infra code makes, against the control plane:
...
defer ts.Close()
// point the same rds client at ts.URL instead of the running endpoint
```
