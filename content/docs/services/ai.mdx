...
Run inference and control-plane calls against **Bedrock**, **SageMaker** (AWS), and **Vertex AI** (GCP) with the real SDKs. Each ships a control plane (model catalog, endpoints, jobs) and a deterministic runtime so inference calls return predictable, echo-based responses — no GPUs, no accounts, no cost.
