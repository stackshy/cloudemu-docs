...
> **SDK-compat status:** _Live._ Real `aws-sdk-go-v2/service/iam`, Azure `Microsoft.Authorization` (RBAC), and GCP IAM clients can point at the [SDK-compat server](/docs/sdk-compat). The full driver semantics (JSON policy evaluation, wildcards, explicit-deny override, instance profiles) are also available through the Portable Go API below.
