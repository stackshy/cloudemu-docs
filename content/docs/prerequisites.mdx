...
---
What you need depends on how you run cloudemu. Running it **in-process as a Go library** (the path this guide starts with) needs Go 1.25+ and a Go SDK. Running it as a [standalone server or in Docker](/docs/standalone-server) needs neither — start the emulator, then point any SDK or CLI, in any language, at its endpoints.

## Required for the in-process (Go) path

These apply when you import cloudemu as a Go library. For the [standalone server or Docker](/docs/standalone-server) path, skip straight to that guide — none of the items below are required there.
