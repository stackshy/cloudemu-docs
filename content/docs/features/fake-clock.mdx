...
---
Advance or set the clock by hand to test TTL expiry, dedup windows, and alarms — no `time.Sleep`.

<FeatureGlyph kind="clock" label="config.WithClock(config.NewFakeClock(t0))" />
