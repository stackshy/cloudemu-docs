...
Nothing to mock. No cloud accounts. No bills. The same SDK calls you'd run against real cloud APIs hit a cloudemu server and get back SDK-decodable responses. This page shows the in-process form (`httptest.NewServer`) for Go tests; to integrate an existing app, run the [Standalone Server](/docs/standalone-server) and override your SDK endpoint.

<Callout type="info">
Want a long-lived, out-of-process server that any language's SDK can point at? See [Standalone Server](/docs/standalone-server).
</Callout>
...
## Multi-region isolation (AWS)

The AWS wire server routes every request to a per-region backend, keyed by the region in the caller's SigV4 credential scope (`Credential=AKID/DATE/REGION/SERVICE/aws4_request`). A region's provider is built lazily on its first request and cached from then on; an unsigned request, or one whose scope names the server's default region, routes to the pre-built default backend — so a single-region workload never touches the mux at all.

- **Regional state is isolated per region.** DynamoDB tables, SQS queues, EC2 instances/VPCs, Lambda functions, and every other regional service each get a fresh, independent backend per region.
- **Global services are shared across every region.** IAM, STS (one session store, so an `ASIA` credential minted in one region verifies in another), Route 53, CloudFront, and Global Accelerator all read and write the *same* underlying state no matter which region's endpoint a request hits — mirroring real AWS, where these services aren't regional. The S3 bucket-name namespace is shared the same way, so bucket names stay globally unique across regions even though each region's S3 backend is otherwise independent.
- **Cost and Resource Explorer aggregate across every region** that's been touched so far, not just the default one.

The testable behavior: a resource created against a `us-east-1`-scoped request is invisible to a `list` call scoped to `eu-west-1` — and vice versa — while an IAM user, role, or hosted zone created in either region is visible from both.
...
- **Signature validation is off by default.** With no flags, requests are accepted regardless of AWS SigV4 / Azure AAD / GCP OAuth signatures, so local development just works. Opt in with `--enforce-auth` — see [Auth Enforcement](/docs/security) for exactly what's verified per provider and its non-guarantees.
