...
- **Signature validation is off by default.** With no flags, requests are accepted regardless of AWS SigV4 / Azure AAD / GCP OAuth signatures, so local development just works. Opt in with `--enforce-auth`: AWS then verifies SigV4 against registered IAM access keys (403 on failure) and enforces IAM authorization for JSON-RPC services, and Azure validates Bearer JWT claims (audience/expiry/principal, 401 on failure). It's a development aid for exercising auth-failure paths, not a hardened security boundary — STS temporary credentials aren't cryptographically verified and GCP/Azure request signatures aren't checked.
