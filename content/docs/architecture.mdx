...
---
Trace a request from an unmodified SDK client down to the in-memory store — and see where to plug in. The primary surface is the **SDK-compatible HTTP server**, which speaks each cloud's real wire protocol; underneath sits a small, deliberately boring three-layer core.
...
The bottom layer contains the actual in-memory implementations for each cloud provider. Each provider (AWS, Azure, GCP) exposes its services as typed fields.
...
type Provider struct {
    ...
    // ... 30+ services in all — IAM, Route53, ELB, ECR, ECS, SNS, EventBridge,
    //     ElastiCache, SecretsManager, CloudWatchLogs, Redshift, SageMaker,
    //     Keyspaces, MemoryDB, NetworkFirewall, SSM, ResourceDiscovery, and more
}
```
...
The `server/` package wraps drivers with handlers that speak each cloud's real wire protocol. The recommended way to use cloudemu is to run this server — as a standalone process or the Docker image — and point a real SDK client at its endpoint:

```go
// Terminal: cloudemu serve  (or docker run -p 4566:4566 ghcr.io/stackshy/cloudemu)
client := s3.NewFromConfig(cfg, func(o *s3.Options) {
    o.BaseEndpoint = aws.String("http://127.0.0.1:4566") // or AWS_ENDPOINT_URL
})
client.PutObject(ctx, &s3.PutObjectInput{ /* real production code */ })
```

Go test suites can host the same server in-process with `httptest.NewServer(awsserver.New(...))` and point the client at `ts.URL` instead — the wire path is identical.
...
For in-process tests or code that doesn't go through an SDK, call the driver directly through the [Portable Go API](/docs/portable-api):
...
- **`topology/`** — cross-service network reachability engine. Consumes compute, networking, and DNS drivers to evaluate `CanConnect`, `TraceRoute`, `Resolve`. See [Network Topology](/docs/topology).
