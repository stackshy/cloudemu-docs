...
<Callout type="warn">
To integrate cloudemu with an existing app, run it in **server mode** and set your SDK's endpoint (`AWS_ENDPOINT_URL` / `BaseEndpoint`, `option.WithEndpoint`, or the Azure ARM override). **Don't write a `_test.go` file for integration** — that tests the library, not your app. The in-process library path (further down) is only for unit tests inside Go code that already imports cloudemu.
</Callout>
