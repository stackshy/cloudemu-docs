...
Many domains have HTTP wire-format handlers across all 3 providers:
...
The remaining 7 domains (DNS, Load Balancer, Notification, Event Bus, Cache, Secrets, Logging) are **available today via the [Portable Go API](/docs/portable-api)**; SDK-compat handlers ship in lockstep across all 3 providers as each domain lands.
