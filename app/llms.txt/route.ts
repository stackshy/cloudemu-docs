...
Run it in server mode and point the running app's SDK endpoint at it — do NOT write a _test.go or use httptest.NewServer to "integrate" it. Start the server (\`docker run -p 4566:4566 -p 4568:4568 -p 4569:4569 ghcr.io/stackshy/cloudemu\`, or \`cloudemu serve\`), then override the endpoint: AWS \`AWS_ENDPOINT_URL\` env or \`o.BaseEndpoint\`; GCP \`option.WithEndpoint(...)\` + \`option.WithoutAuthentication()\`; Azure ARM endpoint/host override. Production points at the real cloud, code unchanged. The in-process httptest path is for Go unit tests only.
